(function ($) {
    ...
    $.fn.treeview = function (settings) {
        var dfop =
            {
                ...
                url: false,
                param: null,
                cbiconpath: "/asset/js/plugins/tree/images/icons/",
        ...
        function buildnode(nd, ht, deep, path, isend) {
            ...
            ht.push("<span data-id='" + nd.id + "' class='bbit-tree-node-text' unselectable='on'>", nd.text, "</span>");
            ht.push("</a>");
            ...
            if (dfop.isTool) {
                ...
                for (var ii in dfop.nodeTools) {
                    ...
                    if (toolItem.node == undefined || toolItem.node == nd.type) {
                        ht.push("<span class='" + toolItem.img + "'  data-id='" + nd.id + "' title='" + toolItem.text + "'></span>");
                    }
                }
                ht.push("</div>");
            }
        ...
        function asnyloadc(pnode, isAsync, callback) {
            if (dfop.url) {
                ...
                $.ajax({
                    ...
                    success: callback,
                    error: function (e) { layermsg("·þÎñ¶ËÎ´ÏìÓ¦¡£", -1); }
                });
            }
        }
    ...
    $.fn.refreshNode = function (value, text) {
        var $id = $(this);
        var id = $id.attr('id').trim();
        var object = $id.find(('#' + id + '_' + value.replace(/-/g, "_"))).find('span[data-id = "' + value + '" ]');
        object.html(text);
    }
