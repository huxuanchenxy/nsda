...
function (a, b) {
    ...
    a.layer = {
        ...
        msg: function (a, d, f) {
            ...
            h && !e.config.skin ? {
                ...
                return d = d || {},
                (-1 === d.icon || d.icon === b && !e.config.skin) && (d.skin = j + " " + (d.skin || "fg-layer-hui")),
                d
            }()))
        },
        load: function (a, b) {
    ...
    f.pt = f.prototype;
    var g = ["fg-layer", ".fg-layer-title", ".fg-layer-main", ".fg-layer-dialog", "fg-layer-iframe", "fg-layer-content", "fg-layer-btn", "fg-layer-close"];
    g.anim = ["fg-anim", "fg-anim-01", "fg-anim-02", "fg-anim-03", "fg-anim-04", "fg-anim-05", "fg-anim-06"],
    f.pt.config = {
        type: 0,
        shade: .3,
        fix: !0,
    ...
    f.pt.vessel = function (a, b) {
        ...
        k = f.title ? '<div class="fg-layer-title" style="' + (i ? f.title[1] : "") + '">' + (i ? f.title[0] : f.title) + "</div>" : "";
        var _length = top.$(".fg-layer-shade").length;
        var _opacity = (f.shade[0] || f.shade);
        if (_length > 0) {
            //_opacity = 0;
        }
        return f.zIndex = h,
        b([f.shade ? '<div class="fg-layer-shade" id="fg-layer-shade' + d + '" times="' + d + '" style="' + ("z-index:" + (h - 1) + "; background-color:" + (f.shade[1] || "#000") + "; opacity:" + _opacity + "; filter:alpha(opacity=" + (100 * f.shade[0] || 100 * f.shade) + ");") + '"></div>' : "", '<div class="' + g[0] + " " + (g.anim[f.shift] || "") + (" fg-layer-" + e.type[f.type]) + (0 != f.type && 2 != f.type || f.shade ? "" : " fg-layer-border") + " " + (f.skin || "") + '" id="' + g[0] + d + '" type="' + e.type[f.type] + '" times="' + d + '" showtime="' + f.time + '" conType="' + (a ? "object" : "string") + '" style="z-index: ' + h + "; width:" + f.area[0] + ";height:" + f.area[1] + (f.fix ? "" : ";position:absolute;") + '">' + (a && 2 != f.type ? "" : k) + '<div class="fg-layer-content' + (0 == f.type && -1 !== f.icon ? " fg-layer-padding" : "") + (3 == f.type ? " fg-layer-loading" + f.icon : "") + '">' + (0 == f.type && -1 !== f.icon ? '<i class="fg-layer-ico fg-layer-ico' + f.icon + '"></i>' : "") + (1 == f.type && a ? "" : f.content || "") + '</div><span class="fg-layer-setwin">' +
        function () {
            var a = j ? '<a class="fg-layer-min" href="javascript:;"><cite></cite></a><a class="fg-layer-ico fg-layer-max" href="javascript:;"></a>' : "";
    ...
    f.pt.creat = function () {
        ...
        switch ("string" == typeof b.area && (b.area = "auto" === b.area ? ["", ""] : [b.area, ""]), b.type) {
            ...
            case 4:
                ...
                b.content = b.content[0] + '<i class="fg-layer-TipsG"></i>',
                b.title = !1,
                b.shade = !1,
                b.fix = !1,
        ...
        function (d, e) {
            ...
            function () {
                ...
                function () {
                    ...
                    h.parents("." + g[0])[0] || (h.show().addClass("fg-layer-wrap").wrap(d[1]), c("#" + g[0] + f).find("." + g[5]).before(e))
                }()
            }() : c("body").append(d[1]),
            a.layero = c("#" + g[0] + f),
    ...
    f.pt.tips = function () {
        ...
        i.where = [function () {
            ...
            j.removeClass("fg-layer-TipsB").addClass("fg-layer-TipsT").css("border-right-color", b.tips[1])
        },
        function () {
            i.tipLeft = i.left + i.width + 10,
            i.tipTop = i.top,
            j.removeClass("fg-layer-TipsL").addClass("fg-layer-TipsR").css("border-bottom-color", b.tips[1])
        },
        function () {
            i.autoLeft(),
            i.tipTop = i.top + i.height + 10,
            j.removeClass("fg-layer-TipsT").addClass("fg-layer-TipsB").css("border-right-color", b.tips[1])
        },
        function () {
            i.tipLeft = i.left - f[0] - 10,
            i.tipTop = i.top,
            j.removeClass("fg-layer-TipsR").addClass("fg-layer-TipsL").css("border-bottom-color", b.tips[1])
        }],
        i.where[k - 1](),
    ...
    f.pt.move = function () {
        ...
        function (a) {
            if (a.preventDefault(), "ok" === c(this).attr("move")) {
                ...
                c("#fg-layer-moves")[0] || c("body").append('<div id="fg-layer-moves" class="fg-layer-moves" style="left:' + f + "px; top:" + h + "px; width:" + i + "px; height:" + j + 'px; z-index:2147483584"></div>'),
                e.move = c("#fg-layer-moves"),
    ...
    f.pt.callback = function () {
        ...
        f.shadeClose && c("#fg-layer-shade" + b.index).on("click",
        function () {
            layer.close(b.index)
        }),
        d.find(".fg-layer-min").on("click",
        function () {
            layer.min(b.index, f),
            f.min && f.min(d)
        }),
        d.find(".fg-layer-max").on("click",
        function () {
            c(this).hasClass("fg-layer-maxmin") ? (layer.restore(b.index), f.restore && f.restore(d)) : (layer.full(b.index, f), f.full && f.full(d))
        }),
    ...
    e.record = function (a) {
        ...
        a.find(".fg-layer-max").addClass("fg-layer-maxmin"),
        a.attr({
            area: b
        })
    },
    ...
    layer.restore = function (a) {
        ...
        b.find(".fg-layer-max").removeClass("fg-layer-maxmin"),
        b.find(".fg-layer-min").show(),
    ...
    layer.close = function (a) {
        ...
        if (b[0]) {
            if (d === e.type[1] && "object" === b.attr("conType")) {
                ...
                for (var f = 0; 2 > f; f++) b.find(".fg-layer-wrap").unwrap().hide()
            } else {
                if (d === e.type[2]) try {
                    var h = c("#" + g[4] + a)[0];
            ...
            c("#fg-layer-moves, #fg-layer-shade" + a).remove(),
