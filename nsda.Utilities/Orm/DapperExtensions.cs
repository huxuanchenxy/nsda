...
namespace Dapper
{
    ...
    public static class DapperExtensions
    {
        ...
        {
            /// <summary>
            /// 插入启用AUTO_INCREMENT，过滤自增主键
            /// 默认true
            /// </summary>
            public static bool IS_INSERT_AUTO_INCREMENT = true;
        ...
        private static string BuildInsertQuery(IDbConnection connection, Type type)
        {
            ...
            if (!_insertQueryCache.TryGetValue(type, out sql))
            {
                ...
                if (Config.IS_INSERT_AUTO_INCREMENT)
                {
                    typeProperties = typeProperties.Where(p => p != keyProperty)
        ...
        public class DefaultKeyPropertyResolver : IKeyPropertyResolver
        {
            ...
            public virtual PropertyInfo ResolveKeyProperty(Type type)
            {
                ...
                if (keyProps.Count > 1)
                {
                    //fix 联合主键表 主键设定为ExplicitKeyAttribute，这里返回第一个
